...
line_bot_api_st = LineBotApi(os.getenv("LINE_CHANNEL_ACCESS_TOKEN"))#自動抓取(load_dotenv已設置完成)
handler_st = WebhookHandler(os.getenv("LINE_CHANNEL_SECRET"))
def linebot(request):
    try:
        body = request.get_data(as_text=True)
        json_data = json.loads(body)                           # json 格式化收到的訊息
        line_bot_api = LineBotApi(line_bot_api_st)  # 輸入 你的 Channel access token
        handler = WebhookHandler(handler_st)         # 輸入 你的 Channel secret
